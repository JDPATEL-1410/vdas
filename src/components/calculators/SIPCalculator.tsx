...
export default function SIPCalculator() {
  ...
  const { invested, total, returns, chartData } = useMemo(() => {
    const data = []
    ...
    for (let y = 1; y <= years; y++) {
      const n = y * 12
      const r = rate / 100 / 12
      cumulativeInvested = amount * n
      cumulativeValue = amount * ((Math.pow(1 + r, n) - 1) / r) * (1 + r)
      data.push({
        year: `Year ${y}`,
      ...
      <div className="flex flex-col lg:flex-row gap-16">
        ...
        <div className="lg:w-1/3 space-y-12">
          <div className="bg-slate-50 p-8 rounded-3xl">
            <h3 className="text-xl font-black text-slate-900 mb-2 tracking-tight">SIP Calculator</h3>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Plan Your Systematic Investments</p>
          </div>

          <div className="space-y-10">
            <div>
              <div className="flex justify-between items-end mb-4">
                <label className="text-xs font-black text-slate-900 uppercase tracking-widest">Monthly Investment</label>
                <span className="text-2xl font-black text-vdas-royal">{formatINR(amount)}</span>
              </div>
              <input type="range" min={500} max={100000} step={500} value={amount} onChange={e => setAmount(+e.target.value)} className="w-full h-1.5 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-vdas-royal" />
            </div>
        ...
        <div className="lg:w-2/3 flex flex-col">
          <div className="grid grid-cols-2 gap-6 mb-12">
            <div className="p-8 bg-slate-50 rounded-3xl border border-slate-100">
              ...
              <p className="text-2xl font-black text-slate-900">{formatINR(invested)}</p>
            </div>
            <div className="p-8 bg-slate-50 rounded-3xl border border-slate-100">
              <p className="text-[10px] font-bold text-vdas-royal uppercase tracking-widest mb-1">Estimated Returns</p>
              <p className="text-2xl font-black text-vdas-royal">{formatINR(returns)}</p>
            </div>
          </div>
          ...
          <div className="flex-1 min-h-[350px]">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData}>
                ...
                />
                <Area type="monotone" dataKey="Invested" stroke="#2563eb" fill="url(#colorInv)" strokeWidth={3} />
                <Area type="monotone" dataKey="Returns" stroke="#fbbf24" fill="url(#colorRet)" strokeWidth={3} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
